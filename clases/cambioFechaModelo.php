...
class cambioFechaModelo extends padreModelo {
...
# $this->garantiareservacion_id = $dato['garantiareservacion_id'];
    ...
    public function registro_amallaves01() {
        $objeto2 = new padreModelo();
    ...
    public function reporte_reservacion02() {
        ...
        if (($data[0]['estadoreservacion_id'] == 2) || ($data[0]['estadoreservacion_id'] == 1)) {

            return $data;
        } else {

            $obj = new padreModelo();
            $sql = "select * from vw_reservacion where reservacion_codigo='" . $data[0]['reservacion_codigo'] . "'  and  estadoreservacion_id in (2,1) ";
            $data = $obj->ejecutar_query($sql);
            return $data;
        }
    }
...
";
    ...
    public function ver_detallecargos() {
        ...
        $obj = new padreModelo();
        $this->buscar_ctaasociadas();
        $sql = "select * from vw_detallereservacion r where ctacliente_id in (" . $this->listadoctaasociada_id . ");";
        $data = $obj->ejecutar_query($sql);
        return $data;
    }
...
union select c.id as ctahuesped from facturacion.ctacliente c where c.id=" . $this->ctahuesped_id . " ";
    ...
    private function buscar_cargos01() {
        $obj = new padreModelo();
        $sql = "select sum(precio) as total from detallereservacion where ctacliente_id in (" . $this->listadoctaasociada_id . ") and cargado is not null UNION select sum(producto_costo) as total from vw_roomservice2 r where ctacliente_id in (" . $this->listadoctaasociada_id . "); ";
        $data = $obj->ejecutar_query($sql);
        $this->abonos = $data[0]['total'];
        return $data[0]['total'];
    }
...
and tarifa_personas='1' ";
    ...
    public function buscar_fechas_extremos() {
      ...
      echo  $sql = "select desde,hasta,id from reservacion where estatu=true and  id = '" . $this->reservacion_id . "'";
        $data0 = $obj->ejecutar_query($sql);
...
#$obj->ejecutar_query($sql);
    ...
    public function reporte_reservacion06_reservacion_id() {
        ...
        if ($data[0]['estadoreservacion_id'] <= 2) {
    ...
    public function verificar_checkin_reservacion() {
        ...
        $obj = new padreModelo();
        $sql = "select checkin from ocupacion where reservacion_id in ('" . $this->reservacion_id . "') and estatu=true";
        $data = $obj->ejecutar_query($sql);
...
/*
  id serial NOT NULL,
  ctacliente_id integer,
  tipopago_id integer,
  tipotarjeta_id integer,
  numerotarjeta character varying(20),
